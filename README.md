...
- **既定のスマートモードは非可逆処理を含みます**。
  拡張子により動画(H.264)・画像(WebP)・音声(Opus)・モデル(量子化)・
  3D(丸め)へ変換される場合があります。重要データは必ず `--safe` を使用し、
  復元後に `sha256sum` で一致を確認してください。
- **INT4/INT8量子化は非可逆です**。「75%削減」は量子化によるもので、
  元の重み値には完全には戻りません(復元dtypeは入力と同一になります)。
- **ベンチマークの適用範囲**: 上記の比較値は3ファイル
  (英語テキスト191KB・JSON1.5MB・SQLite2.1MB)での実測であり、
